/* $Id: get_util.c,v 1.13 2006/04/28 20:02:55 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    An offshoot of LIBALEX.C.  (Wow, most of these functions date back to
    1987 or early 1988 when I stuck a bunch of useful functions in Alex's
    library file, "libalex.c"!  They were similar to utility functions I'd
    frequently used in Fortran on previous projects.)
...
Procedures:
    ...
    getfield_c() - gets the next character-delimited field from a text record.
    getfield_s() - gets the next string-separated field from a text record.
...
Procedure:
    ...
    Get Next Field from Database Dump Record.
...
Purpose:
    ...
    need to manipulate the field yourself; e.g., use strncpym(), strncmp(3),
...
#define  DIVIDER  '|'			/* Divides fields in database dump file. */
    ...
    return (getfield_c (s, DIVIDER, length)) ;
...
Procedure:
    ...
    Get Next Character-Delimited Field from a Text Record.


Purpose:

    Function getfield_c() is used to scan the fields in a string (e.g.,
    a database dump file record) where the fields are delimited by a
    single character.  The following example reads a single record from
    a database dump file and displays each field in the record:
            ...
            fieldLength = 0 ;
            s = getfield (buffer, delimiter, &fieldLength) ;
            while (fieldLength > 0) {
                length = strTrim (s, fieldLength) ;	-- Trim trailing blanks.
                printf ("Field = %.*s\n", length, s) ;
                s = getfield (s, delimiter, &fieldLength) ;
            }

    The function value returned by getfield_c() is a pointer to the start
    of the field in the calling program's string buffer; LENGTH returns the
    length of the field.  NOTE that getfield_c() doesn't null-terminate the
    field - you need to manipulate the field yourself; e.g., use strncpym(),
    strncmp(3), strncpy(3), etc.  Makes me miss FORTRAN-77!


    Invocation:

        char  delimter, *lastField, *nextField, record[N] ;
        int  length ;

        delimter = '|' ;			-- Initial values.
        lastField = record ;
        length = 0 ;
        ...
        nextField = getfield (lastField, delimiter, &length) ;

    where

        <lastField>	- I
            points to the start of the last field returned by getfield_c(),
            when beginning the scan of a record, set LASTFIELD to point to
            the beginning of the buffer being scanned.
        <delimiter>	- I
            is the character used to delimit the fields in the record being
            scanned.
        <length>	- I/O
            on input, is the length of the last field returned by getfield_c();
            set it to 0 when beginning the scan of a record.  On output, LENGTH
            returns the length of the next field, excluding the beginning and
            ending field delimiters (if any).
        <nextField>
            returns a pointer to the start of the next field.  The pointer
            points into the same buffer that LASTFIELD points into; the new
...
{    /* Local variables. */
    ...
    if (*bofield == delimiter)  bofield++ ;

    *length = strlen (bofield) ;		/* Assume no delimiter at end of string. */

    eofield = strchr (bofield, delimiter) ;	/* End of field. */
    if (eofield != NULL)  *length = eofield - bofield ;
...
Procedure:
    ...
    Get Next String-Separated Field from a Text Record.


Purpose:

    Function getfield_s() is used to scan the fields in a record where
    the fields are delimited by a specified string (e.g., " - " in
    "<field> - <field2> - ...").  The following example scans a record
    and displays each field in the record:
            ...
            fieldLength = 0 ;
            s = getfield_s (buffer, separator, &fieldLength) ;
            while (fieldLength > 0) {
                length = strTrim (s, fieldLength) ;	-- Trim trailing blanks.
                printf ("Field = %.*s\n", length, s) ;
                s = getfield_s (s, separator, &fieldLength) ;
            }

    The function value returned by getfield_s() is a pointer to the start
    of the field in the calling program's string buffer; LENGTH returns the
    length of the field.  NOTE that getfield_s() doesn't null-terminate the
    field - you need to manipulate the field yourself; e.g., use strncpym(),
    strncmp(3), strncpy(3), etc.


    Invocation:

        char  *lastField, *nextField, record[N], *separator ;
        int  length ;

        separator = " - " ;			-- Initial values.
        lastField = record ;
        length = 0 ;
        ...
        nextField = getfield_s (lastField, separator, &length) ;

    where

        <lastField>	- I
            points to the start of the last field returned by getfield_s(),
            when beginning the scan of a record, set LASTFIELD to point to
            the beginning of the buffer being scanned.
        <separator>	- I
            is the possibly multi-character string used to separate the fields
            in the record being scanned.
        <length>	- I/O
            on input, is the length of the last field returned by getfield_s();
            set it to 0 when beginning the scan of a record.  On output, LENGTH
            returns the length of the next field, excluding the beginning and
            ending field separators (if any).
        <nextField>
            returns a pointer to the start of the next field.  The pointer
            points into the same buffer that LASTFIELD points into; the new
            field is NOT null-terminated.
...
{    /* Local variables. */
    ...
    bofield = (char *) s + *length ;		/* Beginning of field. */
    if (strncmp (bofield, separator, strlen (separator)) == 0)
        bofield += strlen (separator) ;		/* Skip separator at beginning. */

    *length = strlen (bofield) ;		/* Assume mo separator at end of string. */

    eofield = strstr (bofield, separator) ;	/* End of field. */
    if (eofield != NULL)  *length = eofield - bofield ;
...
Procedure:
    ...
    Get the Next (Possibly Quote-Delimited) Argument from a String.
...
#    if PROTOTYPES
        const  char  *lastArgument,
        const  char  *quotes,
        size_t  *length)
#    else
        lastArgument, quotes, length)
