/* $Id: opt_util.c,v 1.18 2007/07/05 19:15:38 alex Exp $ */
/*******************************************************************************
...
Purpose:
    ...
    would be scanned and parsed as follows using the OPT_UTIL package:
        ...
        opt_init (argc, argv, 0, "ao:{readonly}{object:}", &scan) ;
...
typedef  struct  _OptContext {
    ...
    bool  printErrors ;			/* Should error messages be printed? */
...
#define  I_DEFAULT_GUARD  opt_util_debug

/*!*****************************************************************************
...
Procedure:
    ...
    Create an ARGV Array of Arguments for a Command Line.
...
/* Allocate an ARGV array to hold the arguments. */
    ...
    *argv = (char **) malloc ((*argc + 1) * sizeof (char *)) ;
    if (*argv == NULL) {
        LGE "(opt_create_argv) Error allocating ARGV array of %d elements.\nmalloc: ",
                 *argc) ;
        return (errno) ;
    }
...
/* Scan the command string again, this time copying the arguments into the
   ARGV array. */
    ...
    LGI "(opt_create_argv) %d arguments:\n", *argc) ;
    for (i = 0 ;  i < *argc ;  i++)
        LGI "                      [%d] = \"%s\"\n", i, (*argv)[i]) ;
...
Procedure:
    ...
    Delete an ARGV Array of Arguments.
...
/* Deallocate the ARGV array. */
    ...
    LGI "(opt_delete_argv) Deleted %d arguments:\n", i) ;
...
Procedure:
    ...
    Enable/Disable Display of Error Messages.
...

{
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_errors) NULL scan context: ") ;
        return (errno) ;
    }
...
/* Set the error display flag in the scan context structure. */
    ...
    LGI "(opt_errors) Error messages are %s.\n",
...
Procedure:
    ...
    Get the Next Option/Argument from the Command Line.
...
Purpose:
    ...
    where:
        ...
        <scan>		- I
            is the command line scan context created by OPT_INIT().
        <argument>	- O
...
/*******************************************************************************
    If the current option group begins with '-', then check for the special
    hyphen cases ("-" and "--") and for single-letter or name options.
*******************************************************************************/

        if ((group[0] == '-') && (scan->current < scan->endOfOptions)) {

/* Check for the special options, "-" and "--".  Return "-" as a non-option
   argument.  For the "--" end-of-options indicator, remember the current
   position on the command line so that the remaining arguments are all
...
/* If we're at the beginning of the options group, check if a name option
   was specified.  Using STRNCMP() (length-restricted comparison) to match
   name options means that name options can be abbreviated.  For example,
   expected option "{name}" will be matched by any of the following on the
...
/* If the current option is not one of the allowed single-letter or name
...

/*******************************************************************************
    The current options group does NOT begin with a hyphen.  Check for I/O
    redirection or a non-option argument.
*******************************************************************************/
...
/* This is a non-option argument.  Return the argument to the calling
   routine. */
...

/*******************************************************************************
    This point in the code is reached if a single-letter or name option was
    specified, or if I/O redirection was requested.
*******************************************************************************/


/* If an argument is expected (because the option expects one or if I/O is
   being redirected to a file), then get the argument.  For single-letter
   options and I/O redirection, the argument may be flush up against the
   option (i.e., the argument is the remainder of the current ARGV) or it
   may be separated from the option by white space (i.e., the argument is
   the whole of the next ARGV).  For name options, the argument is always
   found in the next ARGV. */
...
/* If I/O redirection was requested, then redirect standard input or standard
   output to the specified file. */
        ...
        if (scan->optionType == io_redirection) {
            ...
            switch (group[0]) {
#if defined(HAVE_FREOPEN) && !HAVE_FREOPEN
#    warning  opt_util.c: I/O redirection is not supported.
#else
            case '<':
...
Procedure:
    ...
    Return Position in Command Line Scan.
...

{
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_index) NULL scan context: ") ;
        return (-1) ;
    }
...
Purpose:
    ...
    of the scan.  A pointer to the context structure is returned to the
    calling routine for use in subsequent OPT_GET() calls.


    Invocation (with a single option string):

        char  *optionString = "..." ;
        status = opt_init (argc, argv, optionString, NULL, &scan) ;

    Invocation (with an option string array):

        char  *optionList[]  = { "...", "...", ..., NULL } ;
        status = opt_init (argc, argv, NULL, optionList, &scan) ;
    ...
    where:
        ...
        <isList>	- I
            ...
            If ISLIST is false, OPT_INIT()'s next argument should be a simple
            character string specifying the options.  If ISLIST is true,
            OPT_INIT()'s next argument should be an array of character
            strings, each specifying one or more of the allowable options;
            the last element of the array must be a list-terminating NULL
            pointer.
        <optionString>	- I
        <optionList>	- I
            specify the possible command line options.  Use OPTIONSTRING to
            specify all the options in a single string; use OPTIONLIST to
            specify the options in a NULL-terminated array of option strings.
            Within an option string, options are specified as follows:
                    "a"		Single-letter option
                    "b:"	Single-letter option expecting an argument
                    "{flag}"	Name option
                    "{list:}"	Name option expecting an argument
        <scan>		- O
            returns a handle for this command line scan's context.
...
#    if PROTOTYPES
        ...
        OptContext  *scan)
#    else
        argc, argv, optionString, optionList, scan)
...
/* Count the number of strings in the option list. */

    if (optionList != NULL) {
        for (i = 0, numStrings = 0 ;  optionList[i] != NULL ;  i++) {
            numStrings++ ;
        }
...
/* Allocate a scan context block for the new options scan. */

    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_init) NULL scan context: ") ;
        return (errno) ;
    }

    *scan = (OptContext) malloc (sizeof (_OptContext)) ;
    if (scan == NULL) {
        LGE "(opt_init) Error allocating scan context block.\nmalloc: ") ;
        return (errno) ;
    }

    (*scan)->letterOptions = NULL ;
    (*scan)->letterIndex = NULL ;
    (*scan)->numNames = 0 ;
...
/* Allocate a string to hold the single-letter options and an array to hold
   ...
   position in the original options string. */
    ...
    if ((*scan)->letterOptions == NULL) {
        LGE "(opt_init) Error allocating string to hold %d single-letter options.\nmalloc: ",
            numLetters) ;
        return (errno) ;
    }
    ...
    if (numLetters > 0) {
        ...
        if ((*scan)->letterIndex == NULL) {
            LGE "(opt_init) Error allocating array to hold %d single-letter indices.\nmalloc: ",
                numLetters) ;
            return (errno) ;
        }
    }
   ...
   in the original options string. */
    ...
    if ((*scan)->numNames > 0) {
        ...
        if ((*scan)->nameOptions == NULL) {
            LGE "(opt_init) Error allocating array to hold %d name options.\nmalloc: ",
                (*scan)->numNames) ;
            return (errno) ;
        }
        ...
        if ((*scan)->nameIndex == NULL) {
            LGE "(opt_init) Error allocating array to hold %d name indices.\nmalloc: ",
                (*scan)->numNames) ;
            return (errno) ;
        }
...
/* Finally, scan the option strings again, copying single-letter options and
   name options to the appropriate fields in the context structure. */
...
/* If there are name options, then call OPT_RESET() to construct a table
   ...
   abbreviated name options. */
    ...
    if (opt_reset (*scan, argc, argv)) {
        LGE "(opt_init) Error building table of argument lengths.\nopt_reset: ") ;
        return (errno) ;
    }


    LGI "(opt_init) Single-letter options = \"%s\"\n", (*scan)->letterOptions) ;
    for (i = 0 ;  (*scan)->letterOptions[i] != '\0' ;  i++) {
        LGI "(opt_init) Letter option (%d) = '%c'\n",
            (*scan)->letterIndex[i], (*scan)->letterOptions[i]) ;
    }
    for (i = 0 ;  i < (*scan)->numNames ;  i++) {
        LGI "(opt_init) Name option (%d) = \"%s\"\n",
...
Procedure:
    ...
    Return Name of Option Returned by OPT_GET().
...
{    /* Local variables. */
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_name) NULL scan context: ") ;
        return ("") ;
    }
...
{    /* Local variables. */
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_reset) NULL scan context: ") ;
        return (errno) ;
    }
    ...
    if (argv != NULL) {
        ...
        if (scan->numNames > 0) {
            ...
            if (scan->arglen == NULL) {		/* Allocate a new table. */
                ...
                if (scan->arglen == NULL) {
                    LGE "(opt_reset) Error allocating array to hold %d argument lengths.\nmalloc: ",
                             argc) ;
                    return (errno) ;
                }
            }
...
/* Reset the dynamic fields in the scan context structure so that the command
   line will be rescanned from the beginning. */
    ...
    LGI "(opt_reset) Resetting scan %p.\n", (void *) scan) ;
...
Procedure:
    ...
    Set the Position in a Command Line Scan.
...

{
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_set) NULL scan context: ") ;
        return (errno) ;
    }

    if ((newIndex < 1) || (newIndex > scan->argc)) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_set) Invalid index: %d\n", newIndex) ;
        return (errno) ;
    }
...
/* Set the current position of the command line scan so that the new position
   will be examined on the next call to OPT_GET(). */
    ...
    LGI "(opt_reset) New position of scan %p: %d\n", (void *) scan, newIndex) ;
...
{    /* Local variables. */
    ...
    if (scan == NULL) {
        SET_ERRNO (EINVAL) ;
        LGE "(opt_term) NULL scan context: ") ;
        return (errno) ;
    }

    LGI "(opt_term) Terminating scan %p.\n", (void *) scan) ;
...
{    /* Local variables. */
    ...
    opt_init (argc, argv, argv[1], NULL, &context) ;
