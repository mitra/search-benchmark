/* $Id: rex_util.c,v 1.22 2011/07/18 17:48:17 alex Exp $ */
/*******************************************************************************
...
/* Create and initialize a context structure for the regular expression. */
    ...
    *pattern = (CompiledRE) malloc (sizeof (_CompiledRE)) ;
    if (*pattern == NULL) {
        LGE "(rex_compile) Error allocating context node for \"%s\".\nmalloc: ",
            expression) ;
    ...
    if ((*pattern)->expression == NULL) {
        LGE "(rex_compile) Error duplicating pattern string \"%s\".\nstrdup: ",
            expression) ;
        free (*pattern) ;
...
/* Try matching the regular expression with the text following the last match. */
        ...
        LGI "(rex_replace) Result = \"%s\"  Source = \"%s\"\n",
            *result, source) ;
...
/* Allocate a result string of sufficient size to hold the unmatched text and
   ...
   into the result string, leaving a hole for the replacement text. */
        ...
        r = (char *) malloc (resultLength + 1) ;
        if (r == NULL) {
            LGE "(rex_replace) Error allocating %d bytes for result string.\nmalloc: ",
                resultLength) ;
            return (errno) ;
        }
        strncpy (r, *result, matchOffset) ;		/* Pre-match text. */
        strcpy (r + matchOffset + replacementLength,	/* Post-match text. */
                *result + matchOffset + matchLength) ;
        source = *result ;
        *result = r ;
            ...
            case '$':
                ...
                if (isdigit ((unsigned char) (*s))) {
                    ...
                    if ((fieldLength[i] > 0) && (fieldStart[i] != NULL)) {
                        ...
                        if (changeCase < 0)
                            strToLower (r, fieldLength[i]) ;
                        else if (changeCase > 0)
                            strToUpper (r, fieldLength[i]) ;
                        r = r + fieldLength[i] ;
                    }
                    ...
                    if (matchLength > 0) {
                        ...
                        if (changeCase < 0)
                            strToLower (r, matchLength) ;
                        else if (changeCase > 0)
                            strToUpper (r, matchLength) ;
                        r = r + matchLength ;
                    }
                } else {
                    *r++ = '$' ;
...
{    /* Local variables. */
    char  *r, *s ;
    size_t  length ;
    static  char  *regularExpression = NULL ;
...
/* Allocate a string to hold the resulting wildcard RE. */

    length = strlen (wildcard) * 2 + strlen ("^$") + 1 ;
    if (length > maxLength) {
        if (regularExpression != NULL) {
            free (regularExpression) ;
            maxLength = 0 ;
        }
        regularExpression = malloc (length) ;
        if (regularExpression == NULL) {
            LGE "(rex_wild) Error allocating %u-byte RE string.\nmalloc: ",
                length) ;
            return (NULL) ;
        }
        maxLength = length ;
...
/* Convert the wildcard to a regular expression. */
    ...
    memset (regularExpression, '\0', length) ;
    r = regularExpression ;
    ...
    *r++ = '^' ;			/* Anchor match at start-of-line. */
    ...
    for (s = (char *) wildcard ;  *s != '\0' ;  r++, s++) {
        if (*s == '*')
            ...
            strcpy (r++, "\\(") ;	/* Escape parentheses. */
        else if (*s == ')')
            strcpy (r++, "\\)") ;	/* Escape parentheses. */
        else if (*s == '{')
            strcpy (r++, "\\{") ;	/* Escape braces. */
        else if (*s == '}')
            strcpy (r++, "\\}") ;	/* Escape braces. */
        else
...
/* Traverse the graph representation of the regular expressions, attempting to
    ...
    *next_char = first ;
    ...
    LGI "(rex_search) State = %d  First = %d  Count = %d\n",
    ...
    case match:
        if (target[first] == '\0') {
            return (0) ;		/* End of target string - return. */
                   ...
                   ((unsigned char) target[first] == slist[state].x.match_char) ||
...
/* Allocate a stack for simulating the recursive graph search.  The maximum
   ...
   stack should be sufficient.  Keep your fingers crossed! */
    ...
    if ((i > maxStackSize) || (stack == NULL)) {
        ...
        if (stack == NULL) {
            LGE "(rex_search) Error allocating stack storage for \"%s\".\ncalloc: ", target) ;
            return (0) ;
        }
        maxStackSize = i ;
    }
...
/* Traverse the graph representation of the regular expressions, attempting to
    ...
    *next_char = first ;
    ...
    while (state >= 0) {
        ...
        LGI "(rex_search)%*sState = %d  First = %d  Count = %d  Branch = %d  SP = %d%s\n",
            (sp+1)*2, " ", state, first,
        ...
        case alternation:
            if (popped_from_stack) {
                ...
                if (push (state, first, 0, 1)) {
                    LGE "(rex_search) Stack overflow on RE \"%s\", target \"%s\"\n",
